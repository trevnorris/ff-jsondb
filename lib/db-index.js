...
function DBIndex(db) {
  ...
  // TODO(trevnorris): This variable is no longer needed, but performing the
  // resolvePath() is useful to make sure the index is created. Or does it
  // need to be that complicated?
  this.disk_path =
      resolvePath(db.path + '/' + DBINDEX_PATH + '/f').slice(0, -2);
  // Used in reindex() to make sure the db index is not indexed itself.
  this.index_path_regex = new RegExp('^\\' + DBINDEX_PATH + '(\\/.|)$');
  this.indexes = null;
  ...
  // indexes.
  const indexes_str =
      fs.readFileSync(DBINDEX_PATH + '/indexes', { flag: 'a+' }).toString();
...
// Run indexes recursively on all files at key.
DBIndex.prototype.reindex = function reindex(key) {
  let key_path;
  try {
    key_path = fs.realpathSync(this.db.path + '/' + key);
  } catch (err) {
    return { path: key_path, err };
  }
  return reindex_dir(this, key_path);
};


function reindex_dir(db, dir) {
  // Return if the dir matches the index path.
  if (db.index_path_regex.test(dir)) return;
  const ls = (() => {
    try { return fs.readdirSync(dir) } catch (err) { return { path: dir, err }};
  })();
  if (!Array.isArray(ls)) return ls;
  for (let i = 0; i < ls.length; i++) {
    // XXX: Stop using realpath b/c a symlinked folder will drop the path out
    // of the db directory path, you moron!
  }
}
...
DBIndex.prototype._process = function _process(key, json) {
  ...
  for (let i in this.indexes.index_entries) {
    ...
    if ((entry.filter.type === 'string' && entry.filter.value === key) ||
      ...
      const idata = entry.fn.call(this.db, key, json, index_obj);
