...
function DBIndex(db, index_path) {
  if (index_path === undefined)
    index_path = '/.dbindex';
  if (typeof index_path !== 'string')
    throw new TypeError('index_path must be a string');
  if (index_path.charAt(0) !== '/')
    throw new Error('index_path does not start with "/"');
  ...
  this.disk_path = resolvePath(db.path + '/' + index_path + '/f').slice(0, -2);
  this.indexes = null;
...
// Delete a key in the index.
// XXX: Should the index files be automatically deleted as well?
